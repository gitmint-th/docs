...
## ภาษาไทย — การตรวจสอบด้วยสายตา (Visual QA)

เพิ่มเนื้อหาสั้น ๆ นี้เพื่อตอบโจทย์การตรวจสอบด้วยสายตาของการเปลี่ยนแปลงเอกสาร:

1) การตั้งค่าเซิร์ฟเวอร์
  - ตรวจสอบว่าเซิร์ฟเวอร์ dev รันด้วย `npm run dev` (สคริปต์ใช้ `npx mint dev`) และเปิดที่ `http://localhost:3000` โดยค่าเริ่มต้น
  - ถ้าไม่รัน: ติดตั้ง dependency ด้วย `npm i` แล้วรัน `npm run dev` อีกครั้ง
  - ถ้าเซิร์ฟเวอร์ไม่เริ่ม: ดู `package.json` และบันทึกข้อผิดพลาดจากเทอร์มินัลเพื่อตรวจสอบปัญหา (ตัวอย่าง: dependency หายหรือพอร์ตถูกใช้งาน)

2) ขั้นตอนการทดสอบด้วยสายตา (ทำตามแต่ละหน้าเป้าหมาย):
  - นำทางไปยัง `http://localhost:3000/` แล้วเปิดหน้าเป้าหมายโดยตรง เช่น `http://localhost:3000/get_started/first_generation` เพื่อดูผล
  - ถ่ายภาพหน้าจอคุณภาพสูงของหน้า (เดสก์ท็อป และตัวอย่างขนาดมือถือหากเป็นไปได้)
  - ตรวจสอบรายการต่อไปนี้และบันทึกเป็นข้อสังเกต:
    - เนื้อหาและความสมบูรณ์ของข้อความ
    - เลย์เอาต์ และองค์ประกอบไม่ได้ทับซ้อน
    - รูปภาพโหลดถูกต้อง ไม่มีลิงก์รูปภาพเสีย
    - ฟอนต์และการอ่านง่าย
    - การนำทาง (เมนู/ลิงก์) ทำงานหรือไม่
    - การตอบสนองบนอุปกรณ์เคลื่อนที่

3) รายงานผลสำหรับแต่ละหน้า (รวมไว้ใน PR หรือ issue):
  - URL ของหน้าและภาพหน้าจอ
  - ยืนยันว่าเปลี่ยนแปลงแสดงถูกต้องหรือคำอธิบายปัญหา
  - ข้อเสนอแนะการปรับปรุงการออกแบบ (ถ้ามี)
  - การประเมินคุณภาพภาพโดยรวม (ดี / ปานกลาง / ต้องแก้ไข)

4) ถ้าพบปัญหา ให้ระบุ: ตำแหน่งที่พบ, พฤติกรรมที่คาดหวัง, ระดับความรุนแรง (วิกฤต/สำคัญ/เล็กน้อย), และการแก้ไขที่แนะนำ

หมายเหตุ: เมื่อปรับเปลี่ยน MDX หน้าที่มีอยู่ โปรดอย่าลืมอัปเดตไฟล์แปลภาษาที่สอดคล้องกันใน `zh-CN/` และเพิ่ม redirect ใน `docs.json` ถ้ามีการย้าย/เปลี่ยนชื่อไฟล์
